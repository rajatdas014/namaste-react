...
const CategoryList = ({ catogoryListData }) => {
    ...
    //console.log(catogoryListData)
    const dispatch = useDispatch()
    const handlerAddItem = (accItem) => {
        dispatch(addItem(accItem));
    }
    return (
        <div className="category-list">
            {catogoryListData.map((accItem) => (
                <div data-testid="foodItems" key={accItem.card?.info?.id} className="data-list flex justify-between py-2 border-b-[1px] last:border-b-0 w-[100%]">
                    ...
                    <div className="img-holder relative w-[20%]">
                        <button className="absolute bg-black text-white py-0 px-2 mx-auto left-[63%] bottom-[-5px] rounded-lg" onClick={() => handlerAddItem(accItem)}>Add</button>
